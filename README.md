...
管理者が映画・上映スケジュールを登録し、ユーザーが上映スケジュールから最大3席までまとめて予約できます。  
予約情報は上映スケジュール単位で一覧表示・PDF出力・キャンセルが可能です。  
...
- 映画ポスター画像のアップロードおよび管理（未設定時はデフォルト画像を表示）
- 上映スケジュール詳細画面で座席予約状況の集計情報（総座席数 / 予約済み / 空席 / 利用率）を取得するAPI

### ユーザー側
- 上映スケジュールから最大3席までまとめて座席予約
- 予約履歴一覧を上映スケジュール単位で表示
- 予約情報を上映スケジュール単位でPDFとして出力
- 上映スケジュール単位で予約キャンセル
- 映画に対する「いいね」機能
...
### テスト
主要機能に対してLaravelのFeatureテストを実装
- 座席予約の正常系（単一・複数）
- 予約済み座席の排他制御
- 最大3席制限
- 複数席予約時のトランザクション保証
- 異なる上映回の座席に対する不正リクエストの防止

※ フロントエンドだけでなくバックエンド側でも制約を検証し、
不正なリクエストに対しても安全に処理されるよう設計しています。
